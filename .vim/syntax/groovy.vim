" Vim syntax file
" Language:	Groovy
" Maintainer:	Ivan F. Villanueva B. <groovy_vim@artificialidea.com>
" Version: 	0.1.0b
" URL:	        http://www.vim.org/scripts/script.php?script_id=945	
" Last Change:	1/1/2006
" License:       GPL

" This is my very first syntax vim file, I hope to have done it the right way.
" 
" I must thank the author of the first groovy.vim syntax file, which is in:
" http://www.vim.org/scripts/script.php?script_id=945
" 3 reasons to write a new one: to improve it, the lack of license of the first
" one and the willing to learn about syntax in vim.
"
" Relies on html.vim and java.vim
"
...
"
" 3) add this part to recognize by content groovy scripts (no extension needed :-)
"
"         if did_filetype()
"           finish
"         endif
...
" keyword definitions
syn keyword groovyExternal        package
syn match groovyExternal          "\(\s\+import\s\+static\>\).*"
...
syn keyword groovyType		  Integer Double Date Boolean Float String Array Vector List File
...
syn match   groovyTypedef         "\.\_s*\<\(class\)\>"
syn keyword groovyClassDecl         enum
syn match   groovyClassDecl       "^class\>"
syn match   groovyClassDecl       "[^.]\_s*\<\(class\)\>"
...
syn match   groovyVarArg          "\.\.\."
syn match   groovyRange           "\.\."
...
syn region  groovyString          start=+'+ end=+'+ end=+$+ contains=groovySpecialChar,groovySpecialError,@Spell,groovyELExpr
syn region  groovyString  start=/'''/ skip=/\\"|\\'/ end=/'''/ contains=groovyVarInsideString
syn region  groovyString  start=/"""/ skip=/\\"|\\'/ end=/"""/ contains=groovyVarInsideString
"syn region  groovyString      start=/"/ skip=/\\"|\\'/ end=/"/ contains=groovyVarInsideString
"syn region  groovyString      start=/'/ skip=/\\"|\\'/ end=/'/ contains=groovyVarInsideString
syn match   groovyVarInsideString /${\k\+}/ contained
syn region  groovyList            start=/\[/ end=/\]/ contains=ALL
...
" Groovy JDK stuff
syn keyword groovyJDKBuiltin    as def in err out
...
syn keyword groovyJDKMethods    getErr getIn getOut waitForOrKill waitFor execute
...
syn cluster groovyTop add=groovyExternal,groovyError,groovyError,groovyBranch,groovyLabelRegion,groovyLabel,groovyConditional,groovyRepeat,groovyBoolean,groovyConstant,groovyTypedef,groovyOperator,groovyType,groovyType,groovyStatement,groovyStorageClass,groovyAssert,groovyExceptions,groovyMethodDecl,groovyClassDecl,groovyClassDecl,groovyClassDecl,groovyScopeDecl,groovyError,groovyError2,groovyUserLabel,groovyLangObject,groovyAnnotation,groovyVarArg,groovyRange
...
if version >= 508 || !exists("did_groovy_syn_inits")
  ...
  hi def link groovyVarArg		Function
